...
locals {
  vm_name = "loadbalancer-node.qcow2"
}

source "qemu" "loadbalancer-node" {
  accelerator         = "kvm"
  boot_command        = ["<enter>"]
  ...
  output_directory    = "build/loadbalancer-node"
...
build {
  sources = ["source.qemu.loadbalancer-node"]

  provisioner "shell" {
    execute_command = "bash -x {{.Path}}"
    scripts = [
      "./cloud-init/scripts/install-loadbalancer.sh"
    ]
  }
}
